...
const strings = {
  ja: { tagline: '好きなものを、ものさしに。', primary: '撮ったもので測る', reuse: 'また使う', empty: '保存したものはまだありません', standard: 'cm・inchで測る', use: 'これで測る', edit: '編集', remove: '削除', back: 'ホームへ', history: '履歴', name: '名前をつける・変更', save: '保存', deleteQuestion: 'この画像を削除しますか？', cancel: 'キャンセル' },
  en: { tagline: 'Measure with anything.', primary: 'Measure with a photo', reuse: 'Use again', empty: 'Nothing saved yet', standard: 'Measure in cm / inch', use: 'Measure with this', edit: 'Edit', remove: 'Delete', back: 'Home', history: 'History', name: 'Add or change name', save: 'Save', deleteQuestion: 'Delete this image?', cancel: 'Cancel' },
} as const
...
function App() {
  ...
  const [remeasureSource, setRemeasureSource] = React.useState<SavedMeasurement | null>(null)
  const [editName, setEditName] = React.useState('')
  const t = strings[locale]
  const refreshUnits = React.useCallback(async () => { try { setUnits(await listSavedUnits()) } catch { setUnits([]) } }, [])
  React.useEffect(() => { document.documentElement.lang = locale }, [locale])
  React.useEffect(() => { void refreshUnits() }, [refreshUnits])
  const switchLocale = (next: Locale) => { localStorage.setItem('nankobun.locale', next); setLocale(next) }
  const startMeasurement = (unit: SavedUnit) => { setActiveUnit(unit); setAreaTarget(null); setScreen('length-measurement') }
  const openUnitActions = (unit: SavedUnit) => { setActiveUnit(unit); setEditName(unit.name ?? ''); setScreen('unit-actions') }

  if (screen === 'create') return <UnitCreator locale={locale} onClose={() => { setScreen('home'); void refreshUnits() }} onSaved={refreshUnits} onReadyToMeasure={startMeasurement} />
  if (screen === 'unit-actions' && activeUnit) return <main className="editor-screen center-content"><button className="text-button" onClick={() => setScreen('home')}>← {t.back}</button><div className="cutout-preview checker"><img src={activeUnit.imageDataUrl} alt={activeUnit.name || ''} /></div><button className="primary-button" onClick={() => startMeasurement(activeUnit)}>{t.use}</button><label className="field-label">{t.name}<input value={editName} onChange={(e) => setEditName(e.target.value)} /></label><button className="secondary-button" onClick={async () => { const updated = { ...activeUnit, name: editName.trim() || undefined }; await saveUnit(updated); setActiveUnit(updated); await refreshUnits() }}>{t.save}</button><button className="danger-button" onClick={async () => { if (!window.confirm(t.deleteQuestion)) return; await deleteUnit(activeUnit.id); await refreshUnits(); setActiveUnit(null); setScreen('home') }}>{t.remove}</button></main>
  if (screen === 'length-measurement' && activeUnit) return <LengthMeasurement locale={locale} unit={activeUnit} onClose={() => setScreen('home')} onArea={(target) => { setAreaTarget(target); setScreen('area-measurement') }} />
